...
def create_synthetic_json_data(output_dir: Path, n_files: int = 5):
    ...
    In real usage, you would generate these files by running:
        genome_entropy run --genbank genome.gbk --output results/genome.json
    """
    print(f"\n{'='*60}")
    ...
    for file_idx in range(n_files):
        ...
        for orf_idx in range(n_orfs):
            ...
            features[orf_id] = {
                ...
                "dna": {"nt_sequence": "ATG" * (length // 3), "length": length},
                "protein": {"aa_sequence": "M" * (length // 3), "length": length // 3},
                "three_di": {
                    "encoding": "A" * (length // 3),
                    "length": length // 3,
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("\nLabel distribution:")
        print(
            f"  - In GenBank (positive): {(y == 1).sum()} ({(y == 1).sum() / len(y) * 100:.1f}%)"
        )
        print(
            f"  - Not in GenBank (negative): {(y == 0).sum()} ({(y == 0).sum() / len(y) * 100:.1f}%)"
        )
        ...
        train_metrics = classifier.fit(
            X_train, y_train, feature_names=feature_names, validation_split=0.2
        )
        ...
        print(
            f"{'Sample':<10} {'True':^10} {'Predicted':^12} {'Confidence':^12} {'Correct?':^10}"
        )
        print("-" * 60)
        for i, (pred, prob, true) in enumerate(
            zip(predictions, probabilities, y_sample)
        ):
            ...
            print(
                f"{i+1:<10} {true_label:^10} {pred_label:^12} {confidence:^12.4f} {correct:^10}"
            )
        ...
        print("✓ Model can distinguish GenBank annotations from spurious ORFs")
        print("\nNext steps:")
