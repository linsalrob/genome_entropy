...
class XGBoostModel(BaseModel):
    ...
    def fit(
        ...
        logger.info(
            f"Training XGBoost with {len(X_train)} train, {len(X_val)} val samples"
        )
        ...
        logger.info(
            f"Training complete. Val accuracy: {val_accuracy:.4f}, Val AUC: {metrics['val_auc']:.4f}"
        )
...
class NeuralNetModel(BaseModel):
    ...
    def fit(
        ...
        y_train_t = (
            self.torch.tensor(y_train, dtype=self.torch.float32)
            .unsqueeze(1)
            .to(self.device)
        )
        X_val_t = self.torch.tensor(X_val, dtype=self.torch.float32).to(self.device)
        y_val_t = (
            self.torch.tensor(y_val, dtype=self.torch.float32)
            .unsqueeze(1)
            .to(self.device)
        )
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 20 == 0:
                logger.info(
                    f"Epoch {epoch+1}/{self.epochs}: val_loss={val_loss:.4f}, val_acc={val_accuracy:.4f}"
                )
