...
def train_with_file_split(
    ...
    logger.info(f"Training set: {len(X_train)} ORF samples with {len(feature_names)} features")
    logger.info(f"Features: {', '.join(feature_names)}")
    logger.info(f"Label distribution: {(y_train == 1).sum()} in GenBank, {(y_train == 0).sum()} not in GenBank")
    ...
    logger.info(f"Label distribution: {(y_test == 1).sum()} in GenBank, {(y_test == 0).sum()} not in GenBank")
