...
def extract_features(
    ...
    for jd in json_data:
        for data in jd:
            ...
            # Handle old format (for backward compatibility)
                ...
                logger.warning(
                    "Old format detected - consider regenerating with new pipeline"
                )
                ...
                for orf in orfs:
                    try:
                        ...
                        logger.warning(
                            f"Failed to extract features from old format: {e}"
                        )
                        continue
            else:
                logger.warning("Unknown JSON format in data")
                continue
    ...
    logger.info(
        f"Label distribution: {np.sum(y == 1)} in GenBank, {np.sum(y == 0)} not in GenBank"
    )
...
class GenbankClassifier:
    ...
    def fit(
        ...
        if self.model_type == "xgboost":
            ...
            self.model = NeuralNetModel(
                input_dim=X.shape[1], device=self.device, **self.model_kwargs
            )
        else:
    ...
    def load(self, path: Path) -> None:
        ...
        if self.model_type == "xgboost":
            ...
            self.model = NeuralNetModel(
                input_dim=12, device=self.device, **self.model_kwargs
            )
        else:
            raise ValueError(f"Unknown model_type: {self.model_type}")
