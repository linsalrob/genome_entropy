...
def read_fasta(fasta_path: Union[str, Path]) -> Dict[str, str]:
    ...
    with open_func(fasta_path, mode) as f:
        ...
        if current_id is not None:
            sequences[current_id] = "".join(current_seq_parts)
            logger.debug(
                "Read sequence '%s' (length=%d)", current_id, len(sequences[current_id])
            )
...
def write_fasta(
    sequences: Dict[str, str], output_path: Union[str, Path], line_width: int = 80
) -> None:
