...
class MultiGPUEncoder:
    ...
    def __init__(
        ...
        if self.gpu_ids:
            logger.info(
                "Initializing multi-GPU encoding with %d GPU(s): %s",
                len(self.gpu_ids),
                self.gpu_ids,
            )
    ...
    async def encode_batch_async(
        ...
        results = await loop.run_in_executor(
            executor, encoder._encode_batch, batch_seqs
        )
    ...
    async def encode_all_batches_async(
        ...
        try:
            ...
            logger.error(
                "Multi-GPU encoding failed during worker execution: %s",
                e,
                exc_info=True,
            )
            if first_error is None:
    ...
    def _encode_single_gpu_sequential(
        ...
        logger.info(
            "Encoding %d sequences in %d batches (single GPU)",
            total_sequences,
            len(batches),
        )
