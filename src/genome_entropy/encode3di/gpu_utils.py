...
def discover_available_gpus() -> List[int]:
    ...
    if slurm_job_gpus:
        try:
            ...
            logger.info(
                "Discovered %d GPU(s) from SLURM_JOB_GPUS: %s", len(gpu_ids), gpu_ids
            )
            return gpu_ids
    ...
    if slurm_gpus:
        try:
            ...
            logger.info(
                "Discovered %d GPU(s) from SLURM_GPUS: %s", len(gpu_ids), gpu_ids
            )
            return gpu_ids
        except ValueError as e:
    ...
    if cuda_visible:
        try:
            ...
            logger.warning(
                "Failed to parse CUDA_VISIBLE_DEVICES='%s': %s", cuda_visible, e
            )
